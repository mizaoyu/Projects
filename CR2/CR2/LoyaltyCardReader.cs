...
//					}
		...
		public byte[] SendReceive(string input)
		{
			byte[] command = HexStringToByteArray(input);

			// Send command to remote device
			//Console.WriteLine("Sending Command: " + ByteArrayToHexString(command));
			byte[] result = isoDep.Transceive(command);
			//Console.WriteLine("Command returns: " + ByteArrayToHexString(result));
